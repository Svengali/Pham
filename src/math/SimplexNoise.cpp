...
float SimplexNoise::fractal( size_t octaves, float x ) const {
	float output = 0.f;
	float denom = 0.f;
	float frequency = mFrequency( x, 0, 0 );
	float amplitude = mAmplitude( x, 0, 0 );

	for( size_t i = 0; i < octaves; i++ ) {
		output += ( amplitude * noise( x * frequency ) );
		denom += amplitude;

		frequency *= mLacunarity( x, 0, 0 );
		amplitude *= mPersistence( x, 0, 0 );
	}
...
float SimplexNoise::fractal( size_t octaves, float x, float y ) const {
	float output = 0.f;
	float denom = 0.f;
	float frequency = mFrequency( x, y, 0 );
	float amplitude = mAmplitude( x, y, 0 );

	for( size_t i = 0; i < octaves; i++ ) {
		output += ( amplitude * noise( x * frequency, y * frequency ) );
		denom += amplitude;

		frequency *= mLacunarity( x, y, 0 );
		amplitude *= mPersistence( x, y, 0 );
	}
...
float SimplexNoise::fractal( size_t octaves, float x, float y, float z ) const {
	float output = 0.f;
	float denom = 0.f;
	float frequency = mFrequency( x, y, z );
	float amplitude = mAmplitude( x, y, z );

	for( size_t i = 0; i < octaves; i++ ) {
		output += ( amplitude * noise( x * frequency, y * frequency, z * frequency ) );
		denom += amplitude;

		frequency *= mLacunarity( x, y, z );
		amplitude *= mPersistence( x, y, z );
	}
