...
void Log_native( const std::string &str )
{
	static char s_buf[4096];
	
	sprintf( s_buf, "Script: [%s]\n", str.c_str() );
...
void ConfigureEngine(asIScriptEngine *engine)
{
	...
	*/
	...
	r = engine->RegisterGlobalFunction("void Log(string &in)", asFUNCTIONPR(Log_native, (std::string&), void), asCALL_CDECL); assert( r >= 0 );
...
{
public:
	asCOutputStream( string * const pBuffer )
		:
		m_pBuffer( pBuffer )
	{ } 

	void Write(const char *text) { *m_pBuffer += text; }
...
void ExecString( asIScriptEngine * const engine, std::string * const pOutput, const std::string &arg )
{
	asCOutputStream out( pOutput );
	...
	if( r < 0 )
		OutputDebugStringA( "Invalid script statement. \n" );
	else if( r == asEXECUTION_EXCEPTION )
		OutputDebugStringA( "A script exception was raised.\n" );

	out.m_pBuffer->append( s_output );

	OutputDebugStringA( out.m_pBuffer->c_str() );
